                ...
                child: Column(
                  ...
                  children: [
                    SizedBox(
                      height: 100.0.h,
                      child:ListView(
                        children:snapshot.data!.map((data) => CustomListTile(
                          imagePath: 'https://flagsapi.com/${data.state}/flat/64.png',
                          text:  '${data.state}',
                          subtitle: 'Casos Totales${data.totalTestsViral}',
                          aditionalInfo: 'Ultima Modificación ${data.dateModified?.toLocal().toString()??''}',
...
class CustomListTile extends StatelessWidget {
  ...
  const CustomListTile({super.key, required this.imagePath, required this.text, required this.icon,required this.subtitle,required this.aditionalInfo});
  ...
  Widget build(BuildContext context) {
    return Column(
      children: <Widget>[
        ...
        ListTile(
          leading: Image.network(imagePath,errorBuilder: (BuildContext context, Object exception, StackTrace? stackTrace) {
